...
SANITIZED_MODEL = _sanitize_model_name(os.environ.get("MODEL_NAME", VALIDATOR_MODEL))
...
def _log_validation_row(task_name: str, status: str, attempt: int = 0) -> None:
    script_end_time = datetime.now(IST).isoformat()
    script_duration = time.perf_counter() - SCRIPT_START_PERF
    _append_timing_rows([
        {
            "step": "validation",
            "model": os.environ.get("MODEL_NAME", VALIDATOR_MODEL),
            "run_count": RUN_COUNT,
            "task_name": task_name,
            "status": status,
            "attempt": attempt,
...
def _call_llm_for_correction(task: Dict, runtime_error: str, exit_code: int, assets: Dict, attempt: int) -> Dict:
    ...
    try:
        ...
        print(f"[Validator] LLM correction for {task.get('task_name')} took {llm_duration:.2f}s")
    except Exception as e:
        llm_end_time = datetime.now(IST).isoformat()
        llm_duration = time.perf_counter() - llm_start_perf
        script_end_time = datetime.now(IST).isoformat()
...
def validate_and_fix_task(script_path: str, task_info: Dict, scripts_dir: str) -> Dict:
    ...
    if exec_result["exit_code"] == 0:
        ...
        if result_json and result_json.get("task_name") == task_name:
            ...
            _log_validation_row(task_name, "passed", 0)
            return {
                "task_name": task_name,
                "status": "passed",
                "message": message,
        ...
        _log_validation_row(task_name, "invalid_output", 0)
        ...
        _log_validation_row(task_name, "failed", 0)
        # Fall through to retry logic below
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if exec_result["exit_code"] == 0:
            ...
            if result_json and result_json.get("task_name") == task_name:
                ...
                _log_validation_row(task_name, "passed_after_retry", attempt)
    ...
    _log_validation_row(task_name, "failed", MAX_RETRIES)
