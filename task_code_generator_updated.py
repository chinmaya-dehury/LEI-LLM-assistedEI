"""
task_code_generator_updated.py
-------------------
Generate Python task code using the LLM and save outputs.
...
""".strip()
    ...
    # Persist raw: remove this if not needed. it store raw llm outputs
    try:
        os.makedirs(os.path.join(OUTPUT_DIR, "raw_llm_outputs"), exist_ok=True)
        base_name = task_payload.get("tasks", [{}])[0].get("task_name", f"batch_{int(time.time())}")
        with open(os.path.join(OUTPUT_DIR, "raw_llm_outputs", f"{base_name}_raw.txt"),
